...
export function renderHeader() {
  ...
  nav.querySelector(".profile-icon")?.addEventListener("click", toggleProfilePanel);
  nav.querySelector(".logOut")?.addEventListener("click", logoutUser);
}

async function toggleProfilePanel() {
  const existingPanel = document.querySelector(".user-info-section");
  ...
  const responseData = await requestGraphql(queries.userProfile);
  ...
  profilePanel.className = "user-info-section active";
  profilePanel.innerHTML = /* html */ `
    <div class="close">
      <svg xmlns="http://www.w3.org/2000/svg" width="40px" height="40px" version="1.1" viewBox="0 0 32 40" x="0px" y="0px" class="close-sec">
    ...
    <div class="username">
      <h2 class="fullname">${user.attrs.firstName} ${user.attrs.lastName}</h2>
      <p class="nickname">#${user.login}</p>
    </div>
    <div class="info-data">
      <p><span>Cohort:</span> ${user.events?.[0]?.cohorts?.[0]?.labelName || "--"}</p>
      <p><span>Email:</span> ${user.attrs.email || "--"}</p>
      <p><span>Phone Number:</span> ${user.attrs.tel || "--"}</p>
      <p><span>Scholar Level:</span> ${user.attrs.scholarLevel || "--"}</p>
      <p><span>Gender:</span> ${user.attrs.gender || "--"}</p>
      <p><span>City:</span> ${user.attrs.addressCity || "--"}</p>
      <p><span>Address:</span> ${user.attrs.addressStreet || "--"}</p>
      <p><span>Profile:</span> <a href="${APP_CONFIG.api.profileBaseUrl}${user.login}" target="_blank" rel="noreferrer">Open public profile</a></p>
    </div>
  `;

  getAppContainer()?.appendChild(profilePanel);
  profilePanel
    .querySelector(".close-sec")
    ?.addEventListener("click", () => profilePanel.classList.remove("active"));
}
